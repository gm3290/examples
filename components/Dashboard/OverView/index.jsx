...
import { getActiveCarReqAction } from "store/assets/actions"
...
function OverView() {
  ...
  const { selectedCar, loading } = useSelector(getAssetsSelector)
  ...
  useEffect(() => {
    ...
    if (user && isEmpty(selectedCar)) {
      dispatch(getActiveCarReqAction(user?.id))
    }
  }, [user])
      ...
      <div className="LeftContent">
        {selectedCar && (
          <RioLabel
            ...
            label={`#${selectedCar.assetId} ${selectedCar.name}`}
          />
        )}
        {selectedCar && (
          <CarCard
